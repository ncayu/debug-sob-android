...
class HomeRecommendAdapter : BaseQuickAdapter<HomeRecommend.RecommendArticle, BaseViewHolder>(R.layout.item_home_recommend_style1) {
    override fun convert(holder: BaseViewHolder, item: HomeRecommend.RecommendArticle) {
        ...
        //阅读与收藏
        val str = "%s阅读 - %s收藏"
        holder.setText(R.id.tv_views, String.format(str, item.viewCount.toString(), item.thumbUp.toString()))
    }
}